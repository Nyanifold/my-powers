...
将所有板块分为两类：
- **仅有 Core** — 迭代计划中没有任何 Update 阶段
- **多阶段** — 迭代计划中有一个或多个 Update 阶段

### 第 2 步：先处理仅有 Core 的板块

对每个仅有 Core 的板块，**跳过子智能体，不派发**。改为：

1. 将原始 spec 文件直接复制为 `docs/my-powers-output/specs/YYYY-MM-DD-<module>-spec-core.md`（内容完全相同——没有 Update 阶段时，spec-core 就是完整 spec）。
2. 将复制的文件提交 git。
3. **显式声明**每个这样的板块：
   > 「<module-name> 无 Update 阶段，原始 spec 直接复制为 spec-core.md，不派发拆分子智能体。」

只有**多阶段**类的板块才进入以下子智能体派发步骤。

### 第 3 步：并行派发子智能体（仅多阶段板块）
...
**prompt 模板：** 使用 `spec-splitter-prompt.md` 中的模板构造子智能体 prompt。
...
### 第 4 步：并行派发检查子智能体（仅多阶段板块）

所有 splitter 子智能体完成后，对每个**多阶段**板块并行派发一个检查子智能体，使用 `spec-split-checker-prompt.md` 中的模板。仅有 Core 的板块为直接复制，无需检查。
