...
module-brainstorming（模块头脑风暴）
   ...
   产出 docs/my-powers-output/YYYY-MM-DD-modules.md。
   ↓ 用户确认
...
大型项目在推进过程中可能积累大量上下文，导致对话窗口接近上限。

在设计阶段（`module-brainstorming`、`writing-module-specs`、`writing-plans`），每份文档一旦写完即已自成体系。若上下文增长较快，可在**任意两份文档之间**手动新开窗口，无需等待阶段边界。两个尤为干净的断点是：
...
新开对话时，将以下文件一并提供给新会话：原始需求文档，以及 `docs/my-powers-output/` 目录下截至当前阶段已产出的所有文档。
...
| `systematic-debugging` | `systematic-debugging` | 直接移植并适配中文。新增在 `docs/my-powers-output/reports/` 中以当前阶段文件名前缀记录缺陷修复记录的要求 |
...
所有中间文件采用 `YYYY-MM-DD-` 日期前缀。`docs/my-powers-output/` 目录由 my-powers 技能自动管理——请勿手动编辑其中的文件。
